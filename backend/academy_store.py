...
class AcademyStore:
    def __init__(self, state_dir: Path, database_url: str | None = None):
        ...
        try:
            ...
            if is_postgres:
                logger.warning("academy store postgres connection failed (%s), falling back to sqlite", str(exc)[:200])
                state_dir.mkdir(parents=True, exist_ok=True)
                self.url = f"sqlite:///{state_dir / 'academy.sqlite3'}"
                self.engine = create_engine(self.url, connect_args={"check_same_thread": False}, future=True)
                Base.metadata.create_all(self.engine)
                logger.info("academy store ready backend=sqlite (fallback)")
            else:
                raise exc
